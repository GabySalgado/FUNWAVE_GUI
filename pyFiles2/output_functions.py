...
from pyFiles2.PrincipalTab_2 import time_text, plotInt_text
...
from pyFiles2.PrincipalTab_5 import out_options, out_list, plot_time, TimeLimit_text, vmean_cols, umean_cols, surf_cols, u_cols, v_cols, Surfmean_cols, Hsig_cols, Hrms_cols, Havg_cols, PLOT_label, plot_time_label, VIDEO_label
...
def toggle_output_MASK(change):
    if change['new']:   
        # if output variable is turned on, show it in output list
        out_options.append('MASK')
        out_list.options = out_options
    else:
        if 'MASK' in out_options: # if output variable is turned off, remove it from output list
            out_options.remove('MASK') 
            out_list.options = out_options
        else:
            pass
...
# This function shows the post-process window of the type of output that was chosen
    ...
    if change['new'] == 'U Velocity':
        #show
        u_cols.layout.display=''
        PLOT_label.value ='Press Button to Generate U velocity Image at a Specific Time:'
        plot_time_label.value = "Plot U velocity at time:"
        VIDEO_label.value = 'Press Button to Generate U velocity Video:'
        ...
        plot_time.value = 0
        TimeLimit_text.value = 0
        widgets.jsdlink((time_text,'value'),(plot_time,'max'))
        widgets.jsdlink((plotInt_text,'value'),(plot_time,'step'))
        widgets.jsdlink((time_text,'value'),(TimeLimit_text,'max'))   
        widgets.jsdlink((plotInt_text,'value'),(TimeLimit_text,'step'))
        
    elif change['new'] == 'V Velocity':
        #show
        v_cols.layout.display=''
        PLOT_label.value ='Press Button to Generate V velocity Image at a Specific Time:'
        plot_time_label.value = "Plot V velocity at time:"
        VIDEO_label.value = 'Press Button to Generate V velocity Video:'
    ...
    elif change['new'] == 'U Velocity Mean':
        #show
        umean_cols.layout.display=''
        PLOT_label.value ='Press Button to Generate U velocity mean Image:'
    ...
    elif change['new'] == 'V Velocity Mean':
        #show
        vmean_cols.layout.display=''
        PLOT_label.value ='Press Button to Generate V velocity mean Image:'
    ...
    elif change['new'] == 'Surface Elevation':
        #show
        u_cols.layout.display=''
        ...
        TimeLimit_text.min = steady_time.value + T_INTV_MEAN.value     
